...
- [Genesis](brands/Genesis.md) GV90 — 히든 B필러 코치도어, 23.6인치 팝업 OLED, Pleos Connect, 4/6인승·회전식 앞좌석, 12에어백과 Roof Airbag이 양산 사양으로 확정됐다.
...
- [Hyundai](brands/Hyundai.md) 2027 Tucson — 대형 중앙 터치스크린, 둥근 센터 콘솔, 2스포크 스티어링휠 등 차세대 실내 디자인이 공식 공개됐다.
...
- [Mahindra](brands/Mahindra.md) [BE 6 SPORTEQ](models/Mahindra-BE6-SPORTEQ.md) — 3화면 콕핏과 Google Gemini 기반 TEQ_Talk를 양산 사양으로 확정했다.
- [Toyota](brands/Toyota.md) Camry Hybrid — 디지털 계기판 표시 불량 리콜로 클러스터 고장안전과 필수 경고정보 이중화 필요성이 부각됐다.

## 기술 변화 지도

### HMI·디스플레이
- [HMI](topics/HMI.md)는 화면 크기 경쟁에서 **표시 우선순위·상태 전환·물리 조작·AI·캐빈 하드웨어 연동**으로 범위가 확대되고 있다.
...
### 안전·규제·품질
- [자동차안전](topics/자동차안전.md) 이슈는 에어백·벨트뿐 아니라 디지털 클러스터, 후방카메라, 화면 레이어·소프트웨어·커넥터까지 실내 전장 전체로 확대된다.
- Hyundai Tucson 리콜은 계기판 단일 표시점의 고장안전·대체 표시 필요성을 다시 부각시켰다.
- Genesis GV90의 Roof Airbag은 탑승자 보호 영역을 머리 위 공간으로 확대하는 양산 사례다.
...
- Genesis GV90, Mercedes-Benz C-Class, Hyundai Tucson, Acura NEXERA Vision, Lamborghini Revuelto SV, Lucid Gravity GT-S, Ford F-150·F-250 Super Duty는 현재 Brand Wiki/Topic에서 추적하며 후속이 누적되면 독립 Model Wiki 승격을 검토한다.
...
- **WATCH — Roof Airbag**: Genesis GV90에서 양산 적용이 발표된 상부 탑승자 보호 에어백.
